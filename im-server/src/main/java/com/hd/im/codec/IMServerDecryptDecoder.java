 ...
 *
 */
public class IMServerDecryptDecoder extends MessageToMessageDecoder<ByteBuf> {
	@Override
	protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list)
		...
		if (byteBuf.readableBytes() > 0) {
			...
			if (op == HDIMProtocol.HeadType.LOGIN.getNumber()) {
				...
				Attribute<UserSession> session = channelHandlerContext.channel().attr(NettyChannelKeys.USER_SESSION);
				if (session == null) {

				}
				/* 需要进行完全解密 */
				byte[] data = Base64.getDecoder().decode(dst);
				ByteBuf transfer = channelHandlerContext.alloc().buffer(data.length + 1);
